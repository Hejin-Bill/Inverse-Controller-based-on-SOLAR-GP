...
class LocalModels():
    ...
    def partition(self,xnew,ynew):
        for n in range(0,np.shape(xnew)[0],1):
            if self.M > 0:
                w = np.empty([self.M,1]) # (metric for distance ?) kernal value between model center and query point
                ...
                if wnear > self.wgen or self.M > 10:
                    ...
                    self.LocalData[near][2] = (xnew[n]+ self.LocalData[near][2]*nloc)/(nloc+1) # update the center of input
                    self.LocalData[near][3] = (ynew[n]+ self.LocalData[near][3]*nloc)/(nloc+1) # update the center of output
            else:
                wnear = 0
    ...
    def prediction(self,xtest, weighted = True, bestm = 3, Y_prev = []):
        ...
        for n in range(0, np.shape(xtest)[0], 1):
            ...
            for k in range(0, self.M, 1):
                ...
                try:
                    c = self.LocalData[k][2] #1x2 # input center
                    d = self.LocalData[k][3] # output center
